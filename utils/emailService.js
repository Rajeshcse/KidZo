...
export const sendVerificationEmail = async (email, otp, name) => {
  try {
    ...
    const mailOptions = {
      from: `"${process.env.FROM_NAME || 'efish'}" <${process.env.FROM_EMAIL || 'noreply@efish.com'}>`,
      to: email,
      subject: 'Email Verification - efish',
      html: `
        <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto;">
          <h2 style="color: #333;">Hi ${name},</h2>
          <p>Thank you for registering with efish! Please verify your email address.</p>
          ...
          <p>If you didn't create an account with efish, please ignore this email.</p>
...
export const sendPasswordResetEmail = async (email, otp, name) => {
  try {
    ...
    const mailOptions = {
      from: `"${process.env.FROM_NAME || 'efish'}" <${process.env.FROM_EMAIL || 'noreply@efish.com'}>`,
      to: email,
      subject: 'Password Reset Request - efish',
      html: `
        <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto;">
          <h2 style="color: #333;">Hi ${name},</h2>
...
export const sendWelcomeEmail = async (email, name) => {
  try {
    ...
    const mailOptions = {
      from: `"${process.env.FROM_NAME || 'efish'}" <${process.env.FROM_EMAIL || 'noreply@efish.com'}>`,
      to: email,
      subject: 'Welcome to efish!',
      html: `
        <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto;">
          <h2 style="color: #333;">Welcome ${name}! 🎉</h2>
          <p>Your email has been successfully verified. You can now enjoy all features of efish.</p>
